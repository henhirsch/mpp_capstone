...
# add 08005 for Buckley AFB fips
co_row_index1 <- which(master_data2$base == "Buckley AFB")
master_data2[co_row_index1, "fips"] <- "08005"

# add 08041 for Schriever AFB fips
co_row_index2 <- which(master_data2$base == "Schriever AFB")
master_data2[co_row_index2, "fips"] <- "08041"

# add 08041 for Peterson AFB fips
co_row_index3 <- which(master_data2$base == "Peterson AFB")
master_data2[co_row_index3, "fips"] <- "08041"

# add 08041 for Fort Carson fips
co_row_index4 <- which(master_data2$base == "Fort Carson")
master_data2[co_row_index4, "fips"] <- "08041"
```
...
```{r clean and manipulate contracts data}
# duplicate data frame
contracts1 <- contracts0

# add a leading 0 to 4 digit values in prime_award_summary_place_of_performance_county_fips_code column and make it a character variable
contracts1 <- contracts1 %>%
  mutate(prime_award_summary_place_of_performance_county_fips_code = ifelse(nchar(prime_award_summary_place_of_performance_county_fips_code) == 4,
  paste0("0", prime_award_summary_place_of_performance_county_fips_code), prime_award_summary_place_of_performance_county_fips_code))
...
# create small contracts data frame
contracts2_small <- contracts2 %>%
  filter(total_obligated_amount <= 1000000)
...
contracts3_small <- contracts2_small %>%
  group_by(prime_award_summary_place_of_performance_county_fips_code) %>%
  summarize(total_obligated_amount_sum = sum(total_obligated_amount, na.rm = TRUE))
...
contracts3_small <- contracts3_small[!is.na(contracts3_small$prime_award_summary_place_of_performance_county_fips_code), ]

# remove rows where total_obligated_amount_sum = 0
contracts3 <- contracts3[contracts3$total_obligated_amount_sum != 0, ]

contracts3_small <- contracts3_small[contracts3_small$total_obligated_amount_sum != 0, ]
...
```{r merge contracts3 onto master_data2 by fips}
...
```{r create small installations data frames}
...
```{r map small installations (no army) color coded by contract funding}
# set caption text
...
```{r map small installations (no army) color coded by small contract funding}
# plot map
...
```{r FOR POSTER ONLY map small installations (no army) color coded by contract funding}
# plot map
